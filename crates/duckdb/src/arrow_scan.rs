...
use crate::{error::duckdb_failure_from_state, ffi, Connection, Error, Result};
...
impl Connection {
    ...
    pub fn register_arrow_scan_view(&self, view_name: &str, arrow_scan: &FFI_ArrowArrayStream) -> Result<()> {
        ...
        if r != ffi::DuckDBSuccess {
            return Err(duckdb_failure_from_state(
                r,
                Some("duckdb_arrow_scan failed to register view".to_string()),
            ));
        }
        Ok(())
    }
